var CC = (function () {
  ...
  return {
    ...
    // Function to calculate the area of a trapezoid
    trapezoidArea: function (base1, base2, height) {
      validateNumbers(base1, base2, height);
      return 0.5 * (base1 + base2) * height;
    },

    // Function to calculate the perimeter of a trapezoid
    trapezoidPerimeter: function (base1, base2, side1, side2) {
      validateNumbers(base1, base2, side1, side2);
    ...
    rhombusArea: function (diagonal1, diagonal2) {
      validateNumbers(diagonal1, diagonal2);
      return (diagonal1 * diagonal2) / 2;
    },

    // Function to calculate the perimeter of a rhombus
    rhombusPerimeter: function (side) {
      validateNumbers(side);
      return 4 * side;
    },
    ...
    // Function to calculate the surface area of a rectangular prism
    rectangularPrismSurfaceArea: function (length, width, height) {
      validateNumbers(length, width, height);
      return 2 * (length * width + length * height + width * height);
    },
    ...
    isoscelesTriangleArea: function (base, height) {
      validateNumbers(base, height);
      return 0.5 * base * height;
    },

    // Function to calculate the perimeter of an isosceles triangle
    isoscelesTrianglePerimeter: function (base, side) {
      validateNumbers(base, side);
      return 2 * side + base;
    },

    // Function to calculate the area of an equilateral triangle
    equilateralTriangleArea: function (side) {
      validateNumbers(side);
      return (Math.sqrt(3) / 4) * Math.pow(side, 2);
    },

    // Function to calculate the perimeter of an equilateral triangle
    equilateralTrianglePerimeter: function (side) {
      validateNumbers(side);
      return 3 * side;
    },

    // Function to calculate the area of a hexagon
    hexagonArea: function (side) {
      validateNumbers(side);
      return (3 * Math.sqrt(3) / 2) * Math.pow(side, 2);
    },

    // Function to calculate the perimeter of a hexagon
    hexagonPerimeter: function (side) {
      validateNumbers(side);
      return 6 * side;
    },

    // Function to calculate the area of a regular pentagon
    regularPentagonArea: function (side) {
      validateNumbers(side);
      return (0.25 * Math.sqrt(5 * (5 + 2 * Math.sqrt(5)))) * Math.pow(side, 2);
    },

    // Function to calculate the perimeter of a regular pentagon
    regularPentagonPerimeter: function (side) {
      validateNumbers(side);
      return 5 * side;
    },

    // Function to calculate the area of a regular octagon
    regularOctagonArea: function (side) {
      validateNumbers(side);
      return 2 * (1 + Math.sqrt(2)) * Math.pow(side, 2);
    },

    // Function to calculate the perimeter of a regular octagon
    regularOctagonPerimeter: function (side) {
      validateNumbers(side);
      return 8 * side;
    },

    // Function to calculate the area of a regular dodecagon (12-gon)
    regularDodecagonArea: function (side) {
      validateNumbers(side);
      return 3 * Math.sqrt(3) * Math.pow(side, 2);
    },

    // Function to calculate the perimeter of a regular dodecagon (12-gon)
    regularDodecagonPerimeter: function (side) {
      validateNumbers(side);
      return 12 * side;
    },

    // Function to calculate the area of a regular icosagon (20-gon)
    regularIcosagonArea: function (side) {
      validateNumbers(side);
      return 5 / 4 * Math.sqrt(5 * (3 + Math.sqrt(5))) * Math.pow(side, 2);
    },

    // Function to calculate the perimeter of a regular icosagon (20-gon)
    regularIcosagonPerimeter: function (side) {
      validateNumbers(side);
      return 20 * side;
    },

    // Function to calculate the area of a regular polygon given the number of sides (n) and side length
    regularPolygonArea: function (n, side) {
      validateNumbers(n, side);
      const apothem = this.regularPolygonApothem(n, side);
      return (n * side * apothem) / 2;
    },

    // Function to calculate the perimeter of a regular polygon given the number of sides (n) and side length
    regularPolygonPerimeter: function (n, side) {
      validateNumbers(n, side);
      return n * side;
    },

    // Function to calculate the apothem of a regular polygon given the number of sides (n) and side length
    regularPolygonApothem: function (n, side) {
      validateNumbers(n, side);
    ...
    triangleCentroid: function (x1, y1, x2, y2, x3, y3) {
      validateNumbers(x1, y1, x2, y2, x3, y3);
      const centroidX = (x1 + x2 + x3) / 3;
      const centroidY = (y1 + y2 + y3) / 3;
      return { x: centroidX, y: centroidY };
    },

    // Function to calculate the centroid of a quadrilateral given its vertices
    quadrilateralCentroid: function (x1, y1, x2, y2, x3, y3, x4, y4) {
      validateNumbers(x1, y1, x2, y2, x3, y3, x4, y4);
    ...
    polygonCentroid: function (...vertices) {
      ...
      const centroidX = sumX / (numVertices / 2);
      const centroidY = sumY / (numVertices / 2);

      return { x: centroidX, y: centroidY };
    },

    // Function to check if a given point is inside a triangle
    isPointInsideTriangle: function (pointX, pointY, x1, y1, x2, y2, x3, y3) {
      validateNumbers(pointX, pointY, x1, y1, x2, y2, x3, y3);

      const areaOfTriangle = 0.5 * Math.abs(
        x1 * (y2 - y3) +
        x2 * (y3 - y1) +
        x3 * (y1 - y2)
      );

      const areaOfSubTriangle1 = 0.5 * Math.abs(
        pointX * (y2 - y3) +
        x2 * (y3 - pointY) +
        x3 * (pointY - y2)
      );

      const areaOfSubTriangle2 = 0.5 * Math.abs(
        x1 * (pointY - y3) +
        pointX * (y3 - y1) +
        x3 * (y1 - pointY)
      );

      const areaOfSubTriangle3 = 0.5 * Math.abs(
        x1 * (y2 - pointY) +
        x2 * (pointY - y1) +
        pointX * (y1 - y2)
      );

      // If the sum of areas of the sub-triangles is equal to the area of the main triangle, the point is inside
      return areaOfTriangle === areaOfSubTriangle1 + areaOfSubTriangle2 + areaOfSubTriangle3;
    },

    // Function to check if a given point is inside a quadrilateral
    isPointInsideQuadrilateral: function (pointX, pointY, x1, y1, x2, y2, x3, y3, x4, y4) {
      validateNumbers(pointX, pointY, x1, y1, x2, y2, x3, y3, x4, y4);

      const areaOfQuad = this.quadrilateralArea(x1, y1, x2, y2, x3, y3, x4, y4);

      const areaOfSubTri1 = this.triangleArea(pointX, pointY, x1, y1, x2, y2);
      const areaOfSubTri2 = this.triangleArea(pointX, pointY, x2, y2, x3, y3);
      const areaOfSubTri3 = this.triangleArea(pointX, pointY, x3, y3, x4, y4);
      const areaOfSubTri4 = this.triangleArea(pointX, pointY, x4, y4, x1, y1);

      // If the sum of areas of the sub-triangles is equal to the area of the quadrilateral, the point is inside
      return areaOfQuad === areaOfSubTri1 + areaOfSubTri2 + areaOfSubTri3 + areaOfSubTri4;
    },

    // Function to check if a given point is inside a polygon (convex or concave) given its vertices
    isPointInsidePolygon: function (pointX, pointY, ...vertices) {
      const numVertices = vertices.length;
      ...
      for (let i = 0, j = numVertices - 2; i < numVertices; i += 2) {
        const xi = vertices[i];
